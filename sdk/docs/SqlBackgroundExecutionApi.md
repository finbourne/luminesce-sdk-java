...
<a id="cancelQuery"></a>
# **cancelQuery**
> BackgroundQueryCancelResponse cancelQuery(executionId).execute();
...
# **fetchQueryResultCsv**
> String fetchQueryResultCsv(executionId).download(download).sortBy(sortBy).filter(filter).select(select).groupBy(groupBy).limit(limit).page(page).delimiter(delimiter).escape(escape).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    try {
      String result = apiInstance.fetchQueryResultCsv(executionId)
            .download(download)
            .sortBy(sortBy)
            .filter(filter)
            .select(select)
...
> File fetchQueryResultExcel(executionId).sortBy(sortBy).filter(filter).select(select).groupBy(groupBy).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    String groupBy = "groupBy_example"; // String | Groups by the specified fields.   A comma delimited list of: 1 based numeric indexes (cleaner), or repeats of the select expressions (a bit verbose and must match exactly).   e.g. `2,3`, `myColumn`.   Default is null (meaning no grouping will be performed on the selected columns).   This applies only over the result set being requested here, meaning indexes into the \"select\" parameter fields.   Only specify this if you are selecting aggregations in the \"select\" parameter.
    try {
      File result = apiInstance.fetchQueryResultExcel(executionId)
            .sortBy(sortBy)
            .filter(filter)
            .select(select)
            .groupBy(groupBy)
...
> String fetchQueryResultHistogram(executionId, timestampFieldName).startAt(startAt).endAt(endAt).bucketSize(bucketSize).filter(filter).jsonProper(jsonProper).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    try {
      String result = apiInstance.fetchQueryResultHistogram(executionId, timestampFieldName)
            .startAt(startAt)
            .endAt(endAt)
...
> String fetchQueryResultJson(executionId).sortBy(sortBy).filter(filter).select(select).groupBy(groupBy).limit(limit).page(page).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    Integer page = 0; // Integer | 0-N based on chunk sized determined by the limit, ignored if limit < 1.
    try {
      String result = apiInstance.fetchQueryResultJson(executionId)
            .sortBy(sortBy)
            .filter(filter)
            .select(select)
            .groupBy(groupBy)
...
> String fetchQueryResultJsonProper(executionId).download(download).sortBy(sortBy).filter(filter).select(select).groupBy(groupBy).limit(limit).page(page).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    Integer page = 0; // Integer | 0-N based on chunk sized determined by the limit, ignored if limit < 1.
    try {
      String result = apiInstance.fetchQueryResultJsonProper(executionId)
            .download(download)
            .sortBy(sortBy)
            .filter(filter)
            .select(select)
...
> File fetchQueryResultParquet(executionId).sortBy(sortBy).filter(filter).select(select).groupBy(groupBy).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    String groupBy = "groupBy_example"; // String | Groups by the specified fields.   A comma delimited list of: 1 based numeric indexes (cleaner), or repeats of the select expressions (a bit verbose and must match exactly).   e.g. `2,3`, `myColumn`.   Default is null (meaning no grouping will be performed on the selected columns).   This applies only over the result set being requested here, meaning indexes into the \"select\" parameter fields.   Only specify this if you are selecting aggregations in the \"select\" parameter.
    try {
      File result = apiInstance.fetchQueryResultParquet(executionId)
            .sortBy(sortBy)
            .filter(filter)
            .select(select)
            .groupBy(groupBy)
...
> String fetchQueryResultPipe(executionId).download(download).sortBy(sortBy).filter(filter).select(select).groupBy(groupBy).limit(limit).page(page).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    Integer page = 0; // Integer | 0-N based on chunk sized determined by the limit, ignored if limit < 1.
    try {
      String result = apiInstance.fetchQueryResultPipe(executionId)
            .download(download)
            .sortBy(sortBy)
            .filter(filter)
            .select(select)
...
> File fetchQueryResultSqlite(executionId).sortBy(sortBy).filter(filter).select(select).groupBy(groupBy).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    String groupBy = "groupBy_example"; // String | Groups by the specified fields.   A comma delimited list of: 1 based numeric indexes (cleaner), or repeats of the select expressions (a bit verbose and must match exactly).   e.g. `2,3`, `myColumn`.   Default is null (meaning no grouping will be performed on the selected columns).   This applies only over the result set being requested here, meaning indexes into the \"select\" parameter fields.   Only specify this if you are selecting aggregations in the \"select\" parameter.
    try {
      File result = apiInstance.fetchQueryResultSqlite(executionId)
            .sortBy(sortBy)
            .filter(filter)
            .select(select)
            .groupBy(groupBy)
...
# **fetchQueryResultXml**
> String fetchQueryResultXml(executionId).download(download).sortBy(sortBy).filter(filter).select(select).groupBy(groupBy).limit(limit).page(page).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    Integer page = 0; // Integer | 0-N based on chunk sized determined by the limit, ignored if limit < 1.
    try {
      String result = apiInstance.fetchQueryResultXml(executionId)
            .download(download)
            .sortBy(sortBy)
            .filter(filter)
            .select(select)
...
# **getProgressOf**
> BackgroundQueryProgressResponse getProgressOf(executionId, buildFromLogs).execute();
